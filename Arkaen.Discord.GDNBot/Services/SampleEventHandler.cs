...
namespace Discord.NetBot1.Services
{
	internal sealed class SampleEventHandler : IHostedService
	{
		...
		public SampleEventHandler( ILogger<SampleEventHandler> logger, DiscordSocketClient client,
								   CommandService commands, IConfiguration config, IServiceProvider services )
		{
			_logger   = logger;
			_client   = client;
			_commands = commands;
...
#warning Ensure that this configuration variable is set.
			     ...
			     || message.HasMentionPrefix( _client.CurrentUser, ref argPos ) )
			{
				...
				finally
				{
					...
					{
						_logger.LogError( "Error Type: {0}{1}Error Reason: {2}", result.Error, Environment.NewLine,
										  result.ErrorReason );
					}
				}
			}
